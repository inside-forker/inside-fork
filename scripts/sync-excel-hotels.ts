...
function parseHotelsExcel(): HotelItem[] {
  if (!fs.existsSync(EXCEL_PATH)) {
    throw new Error(`Excel file not found at: ${EXCEL_PATH}`);
  }

  const wb = XLSX.readFile(EXCEL_PATH);
  const hotelsMap = new Map<string, HotelItem>();

  // 1. Process 'Hotels - Karachi'
  if (wb.Sheets["Hotels - Karachi"]) {
    const wsHotels = wb.Sheets["Hotels - Karachi"];
    const rowsHotels = XLSX.utils.sheet_to_json(wsHotels, { header: 1 }) as any[][];

    for (let i = 1; i < rowsHotels.length; i++) {
      const r = rowsHotels[i];
      if (!r || r.length < 3) continue;
      const rawName = cleanStr(r[2]);
      if (!rawName || rawName.toUpperCase() === "HOTELS") continue;

      const placeId = cleanStr(r[0]);
      const { lat, lng } = parseLatLng(r[10]);

      const key = placeId || rawName.toLowerCase().replace(/[^a-z0-9]/g, "");
      hotelsMap.set(key, {
        sheet: "Hotels - Karachi",
        placeId,
  ...
  if (wb.Sheets["Raw DATA"]) {
    const wsRaw = wb.Sheets["Raw DATA"];
    const rowsRaw = XLSX.utils.sheet_to_json(wsRaw, { header: 1 }) as any[][];
    ...
    for (let i = 1; i < rowsRaw.length; i++) {
      ...
      if (hotelsMap.has(key)) {
        const existing = hotelsMap.get(key)!;
        if (!existing.address) existing.address = cleanStr(r[2]);
        if (!existing.phone) existing.phone = cleanPhone(r[3]);
        if (!existing.description) existing.description = cleanStr(r[4]);
        if (!existing.website) existing.website = cleanUrl(r[5]);
        ...
        hotelsMap.set(key, {
          ...
          description: cleanStr(r[4]),
          website: cleanUrl(r[5]),
...
async function run() {
  ...
  console.log(`\n======================================================`);
  console.log(`  HOTELS & ACCOMMODATIONS SOURCE OF TRUTH SYNC`);
  ...
  try {
    const items = parseHotelsExcel();
    console.log(`✓ Parsed ${items.length} unique Hotel entries from Excel.`);
    ...
    const hotelsCatId = catMapByName.get("hotels-accommodations") || 139;
    ...
    for (const item of items) {
      ...
      const assignedCatIds = [hotelsCatId, tourismCatId];
      ...
      if (matchedDb) {
        ...
        // Unmatched -> INSERT as new hotel listing
        let baseSlug = cleanSlug(item.name);
        if (!baseSlug) baseSlug = `hotel-${Date.now()}`;
        let slug = baseSlug;
        let suffix = 1;
        ...
        const primaryCatId = hotelsCatId;
    ...
    console.log(`\n======================================================`);
    console.log(`  HOTELS SYNC SUMMARY`);
    console.log(`======================================================`);
    console.log(`• Total Hotels Processed:        ${items.length}`);
    console.log(`• Matched Existing Listings:     ${matchedCount}`);
    console.log(`• Listings Enriched / Updated:   ${updatedCount}`);
    console.log(`• New Hotel Listings Inserted:   ${insertedCount}`);
    if (isApply) {
    ...
    client.release();
    await pool.end();
    console.error("Hotels sync failed:", error);
    process.exit(1);
  }
}
