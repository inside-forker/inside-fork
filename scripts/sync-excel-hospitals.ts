...
function parseHospitalsExcel(): HospitalItem[] {
  if (!fs.existsSync(EXCEL_PATH)) {
    throw new Error(`Excel file not found at: ${EXCEL_PATH}`);
  }

  const wb = XLSX.readFile(EXCEL_PATH);
  const hospitalsList: HospitalItem[] = [];

  const ws = wb.Sheets["HOSPITALS - KARCHI"];
  if (!ws) return [];
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!rawName || rawName.toUpperCase() === "HOSPITALS" || rawName.toUpperCase() === "HOSPITALS ") continue;

    const placeId = cleanStr(r[0]);
    const { lat, lng } = parseLatLng(r[6]);

    hospitalsList.push({
      sheet: "HOSPITALS - KARCHI",
      placeId,
...
async function run() {
  ...
  console.log(`\n======================================================`);
  console.log(`  HOSPITALS & HEALTHCARE SOURCE OF TRUTH SYNC`);
  ...
  try {
    const items = parseHospitalsExcel();
    console.log(`✓ Parsed ${items.length} unique Hospital entries from Excel.`);
    ...
    const hospitalsCatId = catMapByName.get("clinics-hospitals") || 93;
    const wellnessCatId = catMapByName.get("health-wellness") || 77;
    const labsCatId = catMapByName.get("diagnostic-labs-imaging") || 113;
    ...
    for (const item of items) {
      ...
      const assignedCatIds = [hospitalsCatId, wellnessCatId];
      ...
      if (matchedDb) {
        ...
        // Unmatched -> INSERT as new hospital listing
        let baseSlug = cleanSlug(item.name);
        if (!baseSlug) baseSlug = `hospital-${Date.now()}`;
        let slug = baseSlug;
        let suffix = 1;
        ...
        const primaryCatId = hospitalsCatId;
    ...
    console.log(`\n======================================================`);
    console.log(`  HOSPITALS SYNC SUMMARY`);
    console.log(`======================================================`);
    console.log(`• Total Hospitals Processed:     ${items.length}`);
    console.log(`• Matched Existing Listings:     ${matchedCount}`);
    console.log(`• Listings Enriched / Updated:   ${updatedCount}`);
    console.log(`• New Hospital Listings Inserted:${insertedCount}`);
    if (isApply) {
    ...
    client.release();
    await pool.end();
    console.error("Hospitals sync failed:", error);
    process.exit(1);
  }
}
