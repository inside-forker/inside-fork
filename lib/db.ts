...
function resolvePoolMax(): number {
  ...
  // Use 5 in local dev so concurrent requests from mobile & web don't deadlock (DO Postgres limit ~22).
  return 5;
}
...
function createPool() {
  ...
  const p = new Pool({
    ...
    // Release idle connections quickly in prod; keep warm longer in dev to avoid constant SSL handshakes.
    idleTimeoutMillis: isProd ? 5_000 : 30_000,

    // How long (ms) to wait when acquiring a connection from the pool.
    connectionTimeoutMillis: isProd ? 10_000 : 25_000,
