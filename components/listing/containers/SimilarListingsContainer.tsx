...
export async function SimilarListingsContainer({
  ...
  try {
    ...
    const similarIds = similarListings
      .map((l) => Number(l.id))
      .filter((n) => Number.isFinite(n) && n > 0);

    if (similarIds.length > 0) {
      const { rows: images } = await query(
        `SELECT * FROM listing_images WHERE listing_id = ANY($1::bigint[]) ORDER BY display_order ASC`,
        [similarIds],
      );
      ...
      similarListings.forEach((l) => {
        (l as typeof l & { images: typeof images }).images = images.filter(
          (img) => Number(img.listing_id) === Number(l.id),
        );
      });
    }
    ...
    console.error("[SimilarListingsContainer] Error:", error);
    return null;
  }
}
